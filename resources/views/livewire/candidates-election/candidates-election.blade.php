...
<div>
    ...
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            <a href="{{ route('pdf.candidates-election', ['search' => $search, 'school_year' => $schoolYearFilter, 'voting_exclusive' => $votingExclusiveFilter]) }}"
                target="_blank"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                    ...
                    data-lucide="printer"
                    class="lucide lucide-printer size-4 stroke-[1.5] [--color:currentColor] stroke-(--color) fill-(--color)/25 h-4 w-4">
            ...
            <div class="mt-3 w-full sm:ml-auto sm:mt-0 sm:w-auto md:ml-0 mr-2">
                <div class="relative w-64">
                    <select wire:model.live="votingExclusiveFilter"
            ...
            <div class="mt-3 w-full sm:ml-auto sm:mt-0 sm:w-auto md:ml-0 mr-2">
                <div class="relative w-56">
                    <select wire:model.live="schoolYearFilter"
            ...
            <div class="mt-3 w-full sm:ml-auto sm:mt-0 sm:w-auto md:ml-0">
                <div class="relative w-56">
                    <input wire:model.live.debounce.300ms="search"
                        ...
                        type="text" placeholder="Search candidate name, student ID, position...">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        data-lucide="search"
                        class="lucide lucide-search size-4 stroke-[1.5] [--color:currentColor] stroke-(--color) fill-(--color)/25 absolute inset-y-0 right-0 my-auto mr-3 h-4 w-4">
                        ...
                        <div
                            ...
                            <div class="p-5">
                                <div
                                    ...
                                    @php
                                        $placeholderDataUri = "data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22200%22%3E%3Crect fill=%22%23e5e7eb%22 width=%22200%22 height=%22200%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 fill=%22%239ca3af%22 font-size=%2214%22 text-anchor=%22middle%22 dy=%22.3em%22%3ENo photo%3C/text%3E%3C/svg%3E";
                                        $imageSrc = $placeholderDataUri;
                                        $storage = \Illuminate\Support\Facades\Storage::disk('public');
                                        if (!empty($student->profile_image) && $storage->exists($student->profile_image)) {
                                            $imageSrc = route('public.file', ['path' => trim($student->profile_image)]);
                                        } elseif (!empty($student->student_id_image) && $storage->exists($student->student_id_image)) {
                                            $imageSrc = route('public.file', ['path' => trim($student->student_id_image)]);
                                        } else {
                                            ...
                                            if (is_dir($studentImagesPath)) {
                                                ...
                                                if (!empty($profileFiles)) {
                                                    $imageSrc = route('public.file', ['path' => 'student_images/' . basename($profileFiles[0])]);
                                                } else {
                                                    $idPattern = $studentImagesPath . '*_id_*' . $student->id . '*';
                                                    $idFiles = glob($idPattern);
                                                    if (!empty($idFiles)) {
                                                        $imageSrc = route('public.file', ['path' => 'student_images/' . basename($idFiles[0])]);
                                                    }
                                                }
                                            }
                                        }
                                    @endphp
                                    <img class="rounded-lg" src="{{ $imageSrc }}" alt="Candidate Photo" onerror="this.onerror=null;this.src='{{ $placeholderDataUri }}';">
                                    <div
                                ...
                                <div class="mt-5 opacity-70">
                                    {{-- Student ID and email intentionally hidden --}}
                                    <div class="mt-2 flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                            stroke-linejoin="round" data-lucide="bar-chart"
                                            class="lucide lucide-bar-chart size-4 stroke-[1.5] [--color:currentColor] stroke-(--color) fill-(--color)/25 mr-2 h-4 w-4">
                                        ...
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 ml-1">
                                    ...
                                    <div class="mt-2 flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                            stroke-linejoin="round" data-lucide="award"
                                            class="lucide lucide-award size-4 stroke-[1.5] [--color:currentColor] stroke-(--color) fill-(--color)/25 mr-2 h-4 w-4">
                                            <path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"></path>
                                            <path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"></path>
                                            <path d="M4 22h16"></path>
                                            <path
                                                d="M10 14.66V17c0 .55-.47.98-.97 1.21l-1 .42c-.52.22-1.03-.22-1.03-.79 0-.28.12-.54.32-.72L8 16.5l-1.32-1.12c-.2-.18-.32-.44-.32-.72 0-.57.51-1.01 1.03-.79l1 .42c.5.23.97.66.97 1.21v-2.34z">
                                            </path>
                                            <path
                                                d="M14 14.66V17c0 .55.47.98.97 1.21l1 .42c.52.22 1.03-.22 1.03-.79 0-.28-.12-.54-.32-.72L16 16.5l1.32-1.12c.2-.18.32-.44.32-.72 0-.57-.51-1.01-1.03-.79l-1 .42c-.5.23-.97.66-.97 1.21v-2.34z">
                                            </path>
                                        </svg>
                                        <span class="text-sm">Status: </span>
                                        @php $isWinner = ($status === 'win'); @endphp
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ml-1 {{ $isWinner ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                            @if($isWinner)
                                            ...
                                            @else
                                                <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                                        d="M10 18a8 8 0 110-16 8 8 0 010 16zM8.293 7.293a1 1 0 011.414 0L10 7.586l.293-.293a1 1 0 111.414 1.414L11.414 9l.293.293a1 1 0 01-1.414 1.414L10 10.414l-.293.293A1 1 0 018.293 9.293L8.586 9l-.293-.293a1 1 0 010-1.414z"
                                                        clip-rule="evenodd"></path>
                                                </svg>
                                            @endif
        ...
        @else
            <div class="col-span-12 text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">There are currently no candidates with vote counts for any position.
                </p>
            </div>
        @endif
